...
/**
 * Convert a whole USDC amount to 6-decimal fixed-point string.
 * '500' → '500000000'
 * Never uses parseFloat.
 */
function usdcToMicro(wholeUsdc: string): string {
  const parts = wholeUsdc.split('.');
  const whole = parts[0] ?? '0';
  return (BigInt(whole) * 1_000_000n).toString();
}
